...
architecture rtl of gpeg_engine is
  ...
  constant OP_LIMIT         : unsigned(3 downto 0) := b"0010";
  ...
  constant OP_VAR           : unsigned(3 downto 0) := b"1011";
  ...
  -- Action element & array
  type action_elt_t is record
    atype                   : unsigned(0 downto 0);
    capture                 : unsigned(15 downto 0);
    offset                  : unsigned(19 downto 0);
    inp_size_ctr            : unsigned(19 downto 0);
    stacklen                : unsigned(15 downto 0);
  end record;

  constant CTYPE_OPEN       : unsigned(0 downto 0) := b"0";
  constant CTYPE_CLOSE      : unsigned(0 downto 0) := b"1";

  constant AELT0 : action_elt_t := (
    atype        => b"0",
    capture      => X"0000",
    offset       => X"00000",
    inp_size_ctr => X"00000",
    stacklen     => X"0000"
  );

  type action_array_t is array (0 to ACTION_DEPTH-1) of action_elt_t;
  ...
  type ctr_elt_t is record
    counter  : unsigned(7 downto 0);
    stacklen : unsigned(7 downto 0);  -- same width as stacksize
    value    : unsigned(19 downto 0);
  end record;

  constant REG_ELT0 : ctr_elt_t := (
    counter => (others => '0'),
    stacklen => (others => '0'),
    value => (others => '0')
  );

  type ctr_array_t is array (0 to REG_DEPTH-1) of ctr_elt_t;
  ...
  signal inp_size_ctr   : unsigned(19 downto 0) := (others => '0');
  signal bc_size_ctr    : unsigned(19 downto 0) := (others => '0');
  ...
  signal instr_ctr      : unsigned(7 downto 0)  := (others => '0');
  ...
  signal instr_var      : unsigned(15 downto 0) := (others => '0');
  ...
  signal ctr_stacksize : unsigned(8 downto 0) := (others => '0');
  ...
  begin
    ...
    report "EXEC: "
           ...
           & " stacksize=" & integer'image(to_integer(stacksize));
      ...
      if rst = '1' then
        ...
        when S_IDLE =>
          ...
          if start = '1' then
            ...
            bc_size_ctr <= bytecode_size;
            inp_size_ctr <= input_size;
            failed <= '0';
            stacksize <= (others => '0');
            n_instr <= (others => '0');
        ...
        when S_LOAD_OP =>
          instr_ctr <= unsigned(bcode_rdata(27 downto 20));
          ...
          instr_var <= unsigned(bcode_rdata(15 downto 0));
          end_code <= bcode_rdata(23 downto 0);
          --waitforcapturevalue <= false;
        ...
        when S_PUSH_ACTION =>
          if actioncount >= ACTION_DEPTH then
            err_code <= ERR_OVERFLOW;
            state <= S_ERROR;
          else
...
--          elsif opcode = OP_LIMIT
--          then
--
---- limit state machine:
--
--
---- signal: busy comparing capture list and assigning first capture list element
----   signal: assigning the next element from the capture element
----     compare of capturelist element's captureid
----     signal: captureid found (compare to instr_var) waitforcapturevalue <= true
--
--            v_redirected := true;
--            if waitforcapturevalue = true
--            then
--              push_limit <= (
--                capture_as_number
--              );
...
--            if waitforcapturevalue = true
--            then
---- start memcmp with capture_start, capture_length
--            end if;
          ...
          then
            if ctr_stacksize = 0 then
              err_code <= ERR_BYTECODE;
              state <= S_ERROR;
              v_redirected := true;
            else
              ctr_scan_idx <= ctr_stacksize - 1;
        ...
        when S_REG_PUSH =>
          if ctr_stacksize >= REG_DEPTH
          then
            err_code <= ERR_OVERFLOW;
            state <= S_ERROR;
          else
            ctr_mem(to_integer(ctr_stacksize)) <= (
              counter => instr_ctr,
              stacklen => stacksize,
              value => instr_offset
            );
            ctr_stacksize <= ctr_stacksize + 1;
            bc_offset <= bc_offset + 4;
            n_instr <= n_instr + 1;
            state <= S_FETCH_OP;
          end if;
        ...
        when S_CONDJUMP_SEARCH =>
          if ctr_mem(to_integer(ctr_scan_idx)).counter = instr_ctr
             and ctr_mem(to_integer(ctr_scan_idx)).stacklen = stacksize
          then
            ctr_found_idx <= ctr_scan_idx;
            state <= S_CONDJUMP_FOUND;
          elsif ctr_scan_idx = 0
          then
            err_code <= ERR_BYTECODE;
            state <= S_ERROR;
          else
            ctr_scan_idx <= ctr_scan_idx - 1;
          end if;

        -- === CONDJUMP action: decrement+jump or remove+fallthrough ===
        when S_CONDJUMP_FOUND =>
          if ctr_mem(to_integer(ctr_found_idx)).value <= 1
          then
            -- Counter exhausted: fall through, remove register
            bc_offset <= bc_offset + 4;
            ctr_scan_idx <= ctr_found_idx;
            state <= S_REG_REMOVE;
          else
            -- Decrement counter and jump
            ctr_mem(to_integer(ctr_found_idx)).value <=
              ctr_mem(to_integer(ctr_found_idx)).value - 1;
            bc_offset <= instr_offset;
            n_instr <= n_instr + 1;
            state <= S_FETCH_OP;
          end if;
        ...
        when S_REG_REMOVE =>
          if ctr_scan_idx + 1 >= ctr_stacksize
          then
            ...
            ctr_mem(to_integer(ctr_scan_idx)) <=
              ctr_mem(to_integer(ctr_scan_idx + 1));
            ctr_scan_idx <= ctr_scan_idx + 1;
          end if;
