...
#define OPCODE_STRINGS_SWITCH switch (opcode) { \
  ...
  case OPCODE_FAIL: return opcode_strings[ 11 ]; \
  case OPCODE_FAILTWICE: return opcode_strings[ 12 ]; \
  case OPCODE_INTRPCAPTURE: return opcode_strings[ 13 ]; \
  case OPCODE_JUMP: return opcode_strings[ 14 ]; \
  case OPCODE_NOOP: return opcode_strings[ 15 ]; \
  case OPCODE_OPENCAPTURE: return opcode_strings[ 16 ]; \
  case OPCODE_PARTIALCOMMIT: return opcode_strings[ 17 ]; \
  case OPCODE_QUAD: return opcode_strings[ 18 ]; \
  case OPCODE_RANGE: return opcode_strings[ 19 ]; \
  case OPCODE_RET: return opcode_strings[ 20 ]; \
  case OPCODE_SET: return opcode_strings[ 21 ]; \
  case OPCODE_SKIP: return opcode_strings[ 22 ]; \
  case OPCODE_SPAN: return opcode_strings[ 23 ]; \
  case OPCODE_TESTANY: return opcode_strings[ 24 ]; \
  case OPCODE_TESTCHAR: return opcode_strings[ 25 ]; \
  case OPCODE_TESTQUAD: return opcode_strings[ 26 ]; \
  case OPCODE_TESTSET: return opcode_strings[ 27 ]; \
  case OPCODE_TRAP: return opcode_strings[ 28 ]; \
  case OPCODE_VAR: return opcode_strings[ 29 ]; \
}
